...
void OTGenerator::initialize(const std::shared_ptr<network::Network>& net) {
    if (party_id_ == 0) {
...
// sender
void OTGenerator::refill_buffer_sender(const std::shared_ptr<network::Network>& net) {
    ex_ot_sender_->send(net, send_msgs_);
    cur_idx_sender_ = 0;
}

// receiver
void OTGenerator::refill_buffer_receiver(const std::shared_ptr<network::Network>& net) {
...
void OTGenerator::get_nch1_ot(const std::shared_ptr<network::Network>& net, const std::size_t select_size,
