...
public:
    explicit PaillierEngine(std::size_t party);

    ~PaillierEngine() = default;
    ...
    void encode(const std::vector<std::uint64_t>& in, solo::ahepaillier::Plaintext& out) const;

    void encode(const Matrix<std::int64_t>& in, solo::ahepaillier::Plaintext& out) const;

    std::uint64_t decode(const solo::ahepaillier::Plaintext& in) const;

    ByteVector encode(const solo::ahepaillier::BigNum& bn, bool is_n_square) const;

    solo::ahepaillier::BigNum decode(const ByteVector& in) const;

    void bn_to_pt(const std::vector<solo::ahepaillier::BigNum>& in, solo::ahepaillier::Plaintext& out) const;

    void bn_to_ct(const std::vector<solo::ahepaillier::BigNum>& in,
            const std::shared_ptr<solo::ahepaillier::PublicKey>& pk, solo::ahepaillier::Ciphertext& out) const noexcept;

    void add(const solo::ahepaillier::Ciphertext& in_0, const solo::ahepaillier::Ciphertext& in_1,
            solo::ahepaillier::Ciphertext& out) const noexcept;

    void add(const solo::ahepaillier::Ciphertext& in_0, const solo::ahepaillier::Plaintext& in_1,
            solo::ahepaillier::Ciphertext& out) const noexcept;

    void mul(const solo::ahepaillier::Ciphertext& in_0, const solo::ahepaillier::Plaintext& in_1,
            solo::ahepaillier::Ciphertext& out) const noexcept;
...
private:
    PaillierEngine(const PaillierEngine&) = delete;
