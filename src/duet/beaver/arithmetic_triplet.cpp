...
ArithmeticTriplet::ArithmeticTriplet(const std::shared_ptr<network::Network>& net, std::size_t party_id,
        std::shared_ptr<PRNG> prng, const std::shared_ptr<OTGenerator>& ot)
        : party_id_(party_id), ot_(ot), prng_(prng) {
    rand_triplet_buff.resize(kDefaulArithmeticTripletBufferSize);
    refill_rand_triplet_buffer(net);
}

std::vector<std::int64_t> ArithmeticTriplet::get_rand_triplet(const std::shared_ptr<network::Network>& net) {
...
void ArithmeticTriplet::refill_rand_triplet_buffer(const std::shared_ptr<network::Network>& net) {
    gen_rand_triplet(net);
    rand_triplet_idx = 0;
    return;
}

void ArithmeticTriplet::gen_rand_triplet(const std::shared_ptr<network::Network>& net) {
